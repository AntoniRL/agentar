...
# -*- coding: utf-8 -*-
"""
Agentar Log GUI Viewer (dopasowany do logów z POSSITION:: x y)
- WORLD: -1 czarne, 0 białe, >0 białe + żółta cyfra
- pole startowe: tło = kolor teamu
- agent: cyfra (ostatnia liczba z ID, np. .1.1.1 -> 1) w kolorze teamu (baza)
- odwiedzone pola (dla master teamu .1.1): trójkąt w lewym-górnym rogu, jaśniejszy kolor teamu
- historia + tryb tail (na żywo)
"""
...
# Przykład z logów:
# [..] INFO: .1.1.1:: (PRINTING) POSSITION:: 4 0
# oraz ewentualny wariant z nawiasami/komą
AGENT_POS_GENERIC_RE = re.compile(
...
class LogModel:
    ...
    # (x,y) -> team (podświetlenie startu)
    start_cells: Dict[Tuple[int,int], str] = field(default_factory=dict)

def _last_number(s: str) -> Optional[int]:
    m = re.search(r'(\d+)(?!.*\d)', s)
    return int(m.group(1)) if m else None

def _lighten(hex_color: str, factor: float = 1.25) -> str:
    """Rozjaśnij kolor przez mnożenie składowych (z limitem 255)."""
    c = hex_color.lstrip('#')
...
def _darken(hex_color: str, factor: float = 0.6) -> str:
    """Przyciemnij kolor przez mnożenie składowych."""
    c = hex_color.lstrip('#')
...
def parse_log(path: str) -> LogModel:
    ...
    # Pozycje (obsługa 2 formatów)
    for m in AGENT_POS_GENERIC_RE.finditer(text):
        agent_id = m.group(1)
        if m.group(2) and m.group(3):
            x, y = int(m.group(2)), int(m.group(3))
        else:
            x, y = int(m.group(4)), int(m.group(5))
        # spróbuj wywnioskować team z najbliższego znanego (dla .1.1 i podobnych)
        team = None
        # najpierw dopasuj dokładny agent_id
        team = model.agent_teams.get(agent_id)
        # jeżeli brak – spróbuj rodziców .1.1.1 -> .1.1 -> .1
        if not team:
            parts = agent_id.strip('.').split('.')
            for i in range(len(parts)-1, 0, -1):
                parent = '.' + '.'.join(parts[:i])
                if parent in model.agent_teams:
                    team = model.agent_teams[parent]
                    break
        # default (jeśli w logach nie było deklaracji) – zielony
        team = (team or 'GREEN').upper()
        model.events.append(AgentEvent(t=len(model.events), agent_id=agent_id, team=team, x=x, y=y))
...
class WorldCanvas(tk.Canvas):
    def __init__(self, master, cell_size: int = 32, **kwargs):
        ...
        self.agent_positions: Dict[str, Tuple[int,int,str]] = {}  # id -> (x,y,team)
    ...
    def mark_visited_by_team(self, x: int, y: int, team: str):
        # Odwiedziny tylko dla master teamu .1.1 (Twoje wymaganie)
    ...
    def update_agent(self, agent_id: str, x: int, y: int, team: str):
        ...
        # cyfra z identyfikatora (ostatni segment liczbowy)
    ...
    def redraw_cell(self, x:int, y:int):
        ...
        # 2) pole startowe -> tło = dokładny kolor teamu
    ...
    def draw_agent(self, x:int, y:int, team:str, agent_id:str=None):
        ...
        label = str(self.agent_indices.get(agent_id, '1'))  # domyślnie '1' jeśli coś nie wyszło
        # cyfra agenta w kolorze bazowym teamu
        self.create_text((x0+x1)//2, (y0+y1)//2, text=label,
                         fill=base, font=('Helvetica','16','bold'))

# -----------------------------
# Kontroler (historia + tail)
# -----------------------------
        ...
        try:
            with open(self.log_path, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                while not self.tail_stop.is_set():
                    ...
                    mpos = AGENT_POS_GENERIC_RE.search(line)
                    if mpos:
                        agent_id = mpos.group(1)
                        if mpos.group(2) and mpos.group(3):
                            x, y = int(mpos.group(2)), int(mpos.group(3))
                        else:
                            x, y = int(mpos.group(4)), int(mpos.group(5))

                        # ustal team jak przy historii
                        team = self.model.agent_teams.get(agent_id)
                        if not team:
                            parts = agent_id.strip('.').split('.')
                            for i in range(len(parts)-1, 0, -1):
                                parent = '.' + '.'.join(parts[:i])
                                if parent in self.model.agent_teams:
                                    team = self.model.agent_teams[parent]
                                    break
                        team = (team or 'GREEN').upper()
...
def main():
    if len(sys.argv) > 1:
        ...
        # domyślnie spróbuj pliku agentar.log w bieżącym folderze
